<template>
    <div class="container">
        ...
        <van-popup v-model:show="showHospital" position="bottom" :style="{ height: '30%' }">
            <van-picker :columns="HospColumns" @confirm="HospOnConfirm" @cancel="showHospital = false" />
        </van-popup>
...
const showTimeConfirm = (item) => {
    // console.log(item)
    const dateString = item.selectedValues.join('-')
    currentDate.value = dateString
    form.starttime = new Date(dateString).getTime()
